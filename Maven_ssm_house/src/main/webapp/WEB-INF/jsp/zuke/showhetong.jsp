...
<div><br/>
    <form name="hetong" action="/hetong/updatehetong.action">
        ...
        <input type="hidden" name="house_id" value="${hetong.houseId}"/>
        <p>出租方：<span name="chuzu">${hetong.chuzu}</span>（以下简称甲方）；</p>　　　
        <p>承租方：<span name="zuke">${hetong.zuke}</span>（以下简称乙方）。　</p>　
        <p>租赁房屋地址：<span name="zuke">${hetong.houselist.address}</span>　</p>　
        ...
        <p>二、起租时间：<span name="fromdate">${hetong.fromdate}</span>，退租时间：<span name="todate">${hetong.todate}</span>。
            在合同签订之日，乙方需向甲方支付房屋租金￥<span name="price">${hetong.houselist.price}</span>。如续租，请提前3天通知甲方，并重新办理租赁手续。<br/></p>
        　　　　
        <p>三、违约处理：</p>　
        <p>甲方违约：</p>　
